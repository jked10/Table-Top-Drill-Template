...
Rules: exactly ${opts.count} injects, progressing through the phases in order; vary the responsible role across injects; the correct option must reflect the procedure. ALWAYS put the single best option FIRST with "correct": 0 (the app reshuffles option order itself). Keep options concise. Return ONLY the JSON object.`;
  ...
  // Ask the account which models it actually has access to, and pick the best
  // cheap one. Cached after the first success.
  async function discoverModel(key) {
    ...
    if (!res.ok) {
      ...
      let msg;
      if (res.status === 401) msg = "Your API key was rejected — re-copy the whole sk-ant-… key and paste it again.";
      else if (res.status === 403) msg = "This key isn't permitted to use the API — create a fresh key in console.anthropic.com → API keys.";
      else msg = `Couldn't reach Anthropic (error ${res.status}${detail ? ": " + detail : ""}).`;
      const err = new Error(msg); err.userMessage = msg; throw err;
    }
    const data = await res.json();
    const ids = (data.data || []).map(m => m.id);
    if (!ids.length) { const e = new Error("Your account has no available models."); e.userMessage = e.message; throw e; }
    // prefer a cheap/fast model; else first listed
    const pref = ids.find(id => /haiku/.test(id)) || ids.find(id => /sonnet/.test(id)) || ids[0];
    localStorage.setItem("tdf_model", pref);
    return pref;
  }
  ...
  async function callAnthropic(prompt) {
    ...
    if (!model) model = await discoverModel(key);
    let triedDiscover = !savedModel();

    async function once(m) {
    ...
    // saved model no longer valid? re-discover once and retry
    if (res.status === 404 && !triedDiscover) {
      localStorage.removeItem("tdf_model");
      model = await discoverModel(key);
      triedDiscover = true;
      res = await once(model);
    }
    if (res.ok) { const data = await res.json(); return (data.content && data.content[0] && data.content[0].text) || ""; }

    let detail = "";
    try { const j = await res.json(); detail = (j && j.error && j.error.message) || ""; } catch (e) {}
    let msg;
    if (res.status === 401) msg = "Your API key was rejected — re-copy the whole sk-ant-… key and paste it again.";
    else if (res.status === 400 && /credit|balance/i.test(detail)) msg = "Your Anthropic account is out of credit — add credit in console.anthropic.com → Billing.";
    else if (res.status === 429) msg = "Rate-limited or out of credit — wait a moment, or top up credit, then retry.";
